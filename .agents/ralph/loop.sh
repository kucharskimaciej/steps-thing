...
CREATE_PR="${CREATE_PR:-$DEFAULT_CREATE_PR}"
PR_DRAFT="${PR_DRAFT:-$DEFAULT_PR_DRAFT}"
...
commit_loop_state() {
  if [ "$MODE" != "build" ] || [ "$NO_COMMIT" != "false" ]; then
    return 0
  fi
  if ! git -C "$ROOT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    return 0
  ...
  git -C "$ROOT_DIR" commit -m "chore(ralph): update run state"
}

create_pr_after_success() {
  if [ "$MODE" != "build" ] || [ "$NO_COMMIT" != "false" ] || [ "$CREATE_PR" != "true" ]; then
    return 0
  fi
  if ! git -C "$ROOT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    return 0
  fi
  local branch base repo pr_args pr_url
  branch="$(git -C "$ROOT_DIR" branch --show-current)"
  if [ -z "$branch" ]; then
    echo "Skipping PR: detached HEAD."
    return 0
  fi
  if [ "$branch" = "main" ] || [ "$branch" = "master" ]; then
    echo "Skipping PR: current branch is $branch."
    return 0
  fi
  if [ -n "$(git_dirty_files)" ]; then
    echo "Skipping PR: working tree is dirty."
    return 0
  fi
  if ! command -v gh >/dev/null 2>&1; then
    echo "Skipping PR: gh not found."
    return 0
  fi
  repo="$(gh repo view --json nameWithOwner --jq .nameWithOwner 2>/dev/null || true)"
  if [ -z "$repo" ]; then
    echo "Skipping PR: could not resolve GitHub repo."
    return 0
  fi
  base="$(git -C "$ROOT_DIR" symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's#^refs/remotes/origin/##')"
  base="${base:-main}"
  ...
  pr_args=(pr create --repo "$repo" --head "$branch" --base "$base" --title "Ralph run $RUN_TAG" --body "Automated Ralph build run. Iterations: $MAX_ITERATIONS.")
