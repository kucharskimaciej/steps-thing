...
Generate snapshots, thumbnails, and dimensions for newly uploaded videos.
...
4. For each unprocessed video:
   ...
   - Upload generated files under owner-scoped snapshot/thumbnail paths.
...
- Create a step with `{ hash, url }`; processing runs once.
- After processing, the video has `snapshotUrl`, `thumbnailUrl`, `width`, and `height`.
